<!-- Clean + Professional Header -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=soft&color=gradient&height=180&section=header&text=Taya%20Bhavsar&fontSize=48&fontColor=ffffff" />
</p>

# 👋 Hi, I'm Taya Bhavsar 
### **Data & Business Analyst | SQL • Python • R • Tableau | Analytics Storyteller**

I’m an early‑career data/business analyst focused on **operational analytics, regression modeling, dashboard design, and clear, actionable insights**. I enjoy transforming messy datasets into structured stories that help teams make smarter decisions.

I’m currently completing my **Business Analytics certificate at San José State University**, where I lead group analytics projects and specialize in **regression analysis, KPI design, and executive‑ready reporting**.

---

## 🔍 What I’m Working On
- Building polished, business‑focused analytics projects  
- Strengthening SQL intuition with real retail & operations datasets  
- Designing interactive Tableau dashboards  
- Practicing clear, concise communication for job fairs and interviews  

---

## 🛠️ Technical Skills

### **Languages & Tools**
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-005C84?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoft-excel&logoColor=white" />
  <img src="https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white" />
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black" />
</p>

---

## 🧠 Top Skills  
*A mix of technical depth, analytical thinking, and business-focused execution.*

### **Core Analytics**
- Regression Analysis (Linear, Multiple, Diagnostics)
- Exploratory Data Analysis (EDA)
- KPI Design & Executive Reporting
- Data Cleaning & Feature Engineering
- Business Insights & Storytelling

### **Technical Tools**
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-005C84?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoft-excel&logoColor=white" />
  <img src="https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white" />
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black" />
</p>

### **Strengths in Practice**
- Translating statistical results into clear, human-readable insights  
- Designing dashboards with clean visuals and intuitive flow  
- Structuring projects with professional documentation  
- Troubleshooting technical issues methodically  
- Communicating findings confidently in group and recruiter settings

  ---
  
## 📌 Featured Projects  
*A blend of technical depth, clean visuals, and business-focused insights.*

### **📊 Advertising Spend Regression Analysis**
Multiple regression model analyzing how TV, radio, and newspaper spend drive sales.  
Includes diagnostics, visualizations, and business recommendations.

<p align="center">
  <img src="https://github.com/tayabhavsar/Advertising-Data-Analysis-Using-Multiple-Regression/blob/main/figures/advertising_plot.png" width="500"/>
</p>

---

### **🍷 Wine Quality Prediction (ML)**
Classification models predicting wine quality using chemical properties.  
Compared algorithms, evaluated performance, and visualized key drivers.

<p align="center">
  <img src="https://github.com/tayabhavsar/Wine-Quality-Prediction/blob/main/figures/wine_correlation_heatmap.png" width="500"/>
</p>

---

### **🚗 U.S. Electric Vehicle Market Share Analysis**
Exploratory analysis of EV adoption trends, market share shifts, and state‑level patterns.  
Includes visual storytelling and KPI summaries.

<p align="center">
  <img src="https://github.com/tayabhavsar/US-Electric-Vehicle-Market-Share-Analysis/blob/main/figures/ev_map.png" width="500"/>
</p>

---

## 🎯 What I’m Looking For
Actively seeking **Data Analyst** or **Business Analyst** roles in:  
- Operations  
- Product analytics  
- Healthcare  
- Payments  
- Retail & supply chain  

Open to **Washington, DC**, **Bay Area**, and **remote/hybrid** roles.

---

## 📫 Let’s Connect
<p>
  <a href="https://www.linkedin.com/in/tayabhavsar">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" />

